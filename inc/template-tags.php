...
function cloutierremix2_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'cloutierremix2' ); ?></h1>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'cloutierremix2' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'cloutierremix2' ) ); ?></div>
			<?php endif; ?>
...
function cloutierremix2_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'cloutierremix2' ); ?></h1>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Previous post link', 'cloutierremix2' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'cloutierremix2' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function cloutierremix2_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'cloutierremix2' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'cloutierremix2' ),
...
function cloutierremix2_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && cloutierremix2_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'cloutierremix2' ) . '</span>', $categories_list );
		}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'cloutierremix2' ), __( '1 Comment', 'cloutierremix2' ), __( '% Comments', 'cloutierremix2' ) );
		echo '</span>';
	}
